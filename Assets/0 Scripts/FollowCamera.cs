...
public class FollowCamera : MonoBehaviour
{
    ...
    void ArrowFollowPosEnemy()
    {
        ...
        Vector2 posPlayer = camCheckEnemy.WorldToScreenPoint(GameManager.Instance.PosEnemy[0].position);

        for (int i = 1; i < 10; i++)
        {
            Vector2 posDirEnemy = camCheckEnemy.WorldToScreenPoint(GameManager.Instance.PosEnemy[i].position);
            ...
            {
                GameManager.Instance.Enemys[i - 1].SetArrowSelf(true);
            }
            else
            {
                GameManager.Instance.Enemys[i - 1].SetArrowSelf(false);
            }
